	...
	while (getline(MyReadFile,inputText)) {
		...
		if (howMany != 0)
		{
			for (int k = 1; k <= howMany; k++)
			{
				...
				//on retrieve
				if (from == 1) stack1 = retrieve(stack1);
				if (from == 2) stack2 = retrieve(stack2);
				if (from == 3) stack3 = retrieve(stack3);
				if (from == 4) stack4 = retrieve(stack4);
				if (from == 5) stack5 = retrieve(stack5);
				if (from == 6) stack6 = retrieve(stack6);
				if (from == 7) stack7 = retrieve(stack7);
				if (from == 8) stack8 = retrieve(stack8);
				if (from == 9) stack9 = retrieve(stack9);
...
std::string PartOne::retrieve(std::string stack) {
	int lenghtInput = stack.length();
	std::string temp = "";

	movedLetter = stack[0];

	for (int i = 1; i < lenghtInput; i++)
	{
		temp += stack[i];
	}
	stack = "";
...
std::string PartOne::add(std::string stack, char letter)
{
	int lenghtInput = stack.length();
	std::string temp = "";
