...
void PartOne::openFile() {
	...
	while (getline(MyReadFile,inputText)) {
		...
		if (isFullyContained(un, deux, trois, quatre))
		{
			totalsum += 1;
		}
...
bool PartOne::isFullyContained(int one, int two, int three, int four)
{
	//si premier range englobe deuxieme range (one est plus petit que three et two est plus grand que four)
	//OU
	//si deuxieme range englobe premier range (one est plus grand que three et two est plus petit que four)
	//alors is fully contained
	if ((one <= three && two >= four) || (one >= three && two <= four)) return true;
